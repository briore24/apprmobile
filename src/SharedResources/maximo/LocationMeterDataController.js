class LocationMeterDataController {
  ...
  onAfterLoadData(datasource, items) {
    if (datasource.name === "maxlib_locationmeters") {
      items.forEach((item) => {
        item.isdelta = item.readingtype_maxvalue === "DELTA";
        if (item.isdelta && this.app.device.isMaximoMobile) {
...
export default LocationMeterDataController;
