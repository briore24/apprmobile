...
public class CalibrationHistoryTest extends AnalogDatasheetTests {
	
	private final Logger logger = LoggerFactory.getLogger(CalibrationHistoryTest.class);
		...
		try {
			if (assetChevron) {
				...
				if (checkAssetDetailPage) {
					this.verifyCalibrationSections(woNum, assignedWorkOrderDetailsPage);
				} else {
