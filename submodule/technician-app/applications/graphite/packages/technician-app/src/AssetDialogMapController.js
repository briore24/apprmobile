import { BaseDialogMapController, AssetMapUtils } from "@maximo/map-component";
...
class AssetDialogMapController extends BaseDialogMapController{
    MAPDS = "maxlib_asset_map_layer_datasource";
    LOOKUPDS = "assetLookupDS";
    TAG = "AssetDialogMapController";
    ...
    /**
     * Selects an asset from the map and sets it as the asset of the specified datasource.
     * @param {string} dsName - The name of the datasource to update.
     * @returns {void}
     */
    async selectAssetFromMap(dsName) {
        const datasource = this.app.findDatasource(dsName);
        let lookupItem = null;
        /* istanbul ignore else */
        if(datasource) {    
            datasource.item[this.app.state.map.selectedDSAttribute] = "" + this.app.state.map.selectedItem.assetnum;

            const lookupDS = this.app.findDatasource(this.LOOKUPDS);
            /* istanbul ignore else */
            if(lookupDS) {
                lookupDS.setQBE("assetnum", "=", this.app.state.map.selectedItem.assetnum);
                ...
                if(lookupDS.items.length === 1) {
                    ...
                    this.app.log.w(this.TAG, "Did not find a single lookup item for asset " + this.app.state.map.selectedItem.assetnum + ". Selected map asset should also be within lookup DS.");
                }
            } else {
            ...
            if(this.page) {
                ...
                if(controller && lookupItem) {
                    controller.chooseAssetItem(lookupItem);
                }
            }
        } else {
            this.app.log.e(this.TAG, 'Did not find asset based datasource.');
        }
    ...
    */
    loading(isLoading, datasource) {
        ...
        if(datasource.name === this.MAPDS && isLoading === false && this.mapUtils) {
            ...
            if(selectedDatasource) {
                let assetVal = selectedDatasource.item[this.app.state.map.selectedDSAttribute];
                /* istanbul ignore else */
                if(assetVal) {
                    //Finds the individual asset record
                    const mapDS = this.app.findDatasource(this.MAPDS);
                    /* istanbul ignore else */
                    if(mapDS) {
                        let foundAssetRecord = mapDS.items.find((item) => {
                            return item.assetnum === assetVal && item.siteid === selectedDatasource.item.siteid;
                        });
                        /* istanbul ignore else */
                        if(foundAssetRecord) {
                            this.app.state.map.selectedItem = foundAssetRecord;
                            let centerCoordinates = this.mapUtils.getCoordinatesFromItem(foundAssetRecord);
                            ...
                            this.app.log.w(this.TAG, "Did not find asset record in map layer data source.  This could either be that it does not exist in map layer data source or the filter is wrong.")
                        }
                    } else {
                        this.app.log.e(this.TAG, 'Did not find the map layer data source.');
                        return;
                    }
                } else if(assetVal === undefined) {
                    this.app.log.w(this.TAG, 'Asset Val is undefined.  Most likely because selected attribute ' + this.app.state.map.selectedDSAttribute +  " is not a property defined on datasource " + selectedDatasource.name);
                }
            } else {
                this.app.log.e(this.TAG, 'Did not find asset based datasource.');
                return;
        ...
        if(datasource.name === this.FILTERDS && this.mapUtils && selected) {
            ...
            if(item) {
                ...
                let foundFeature = features.find((feature) => {
                    ...
                    return maximoAttributes.assetuid === item.assetuid;
                });
                /* istanbul ignore else */
                if(foundFeature) {
     ...
     */
    onAssetMapCreate(map) {
        this.mapUtils = new AssetMapUtils(map);
        this.onMapCreate(map);
    }

    /**
     * Retrieve asset legends.
    ...
    /**
     * Creates a asset symbology based on the provided features and legends.
     * @param {Object} params - The parameters for creating the asset symbology.
     */
    createAssetSymbology(params) {
        return this.mapUtils.createSymbology(params.features, params.legends);
    }
    /**
     * Handle asset map click event.
     * @param {object} item - The clicked item on the map.
     */    
    handleAssetMapClick(item) {
     ...
     * @returns {void}
     */
    handleAssetMapMove(mapInfo) {
        this.handleMapMove(mapInfo);
    }
}

export default AssetDialogMapController;